...
function MeetingUI(props) {
    ...
    useEffect(() => {
        ...
        if (socket) {
            ...
            //socket for Subtitle
            socket.on('subtitle-data', ({ sttSentence, userName, isSubtitle, remoteUserId, subtitle, subtitleFlag, subtitleSegment, remoteUserProfile }) => {
                    ...
                    if(subtitle){
                        if(captions.length > 0){
                            ...
                            if(captions[captions.length - 1].user_id === remoteUserId ) {
                                ...
                                captions.push({'user_id':remoteUserId,'username':userName,'captions':sttSentence, 'currentSubtitle':subtitle, 'subtitleFlag':subtitleFlag, 'subtitleSegment':subtitleSegment, 'userProfile':remoteUserProfile});
                            } 
                ...
                $(".cl_main_noti_box").animate({ scrollTop: $('.cl_main_noti_box').prop("scrollHeight") }, 0);
            });
            ...
            socket.on("new-message", ({ message, messagetime, name }) => {
                console.log('captions in subtitle-data new-message : ',captions);
                    ...
                    $('.cl_user_list').append(
                        `<div class="cl_user_list_box" id='remote-user-${participant}'>
                            ...
                            <div class="cl_user_list_box_control">
                                ...
                                <h4>${participantName[1]} ${participantName[0]=='Teacher'?' (선생님)': participantName[0]=='Student'?' (학생)':''}</h4>                        
                            </div>
                        </div>`
                        );
                } else {
                        ...
                        $('.cl_user_list').append(
                            `<div class="cl_user_list_box" id='remote-user-${participant}'>
                                ...
                                <div class="cl_user_list_box_control">
                                    ...
                                    <h4 style="color:#1C98E5">${participantName[1]} (선생님)</h4>                      
                                </div>
                            </div>`
                            );
                    } else {
    ...
    */    
    ...
    const validateRoom = async () => {
        ...
        try {
            ...
            if (validateRoomResponse.data.Status === "200") {
                ...
                if(meetingUsers && meetingUsers.indexOf(userId) == -1 && userRole != "Admin"){
    ...
    const deleteDocument = async (documentId) => {
     ...
     const subtitleHandler = async () => {
        ...
        socket.emit('subtitle', {'sttSentence':'Muted', 'userName':'', 'isSubtitle':!isSubtitle, 'remoteUserId':userId, 'subtitle':'', 'subtitleFlag':'', 'subtitleSegment':'','remoteUserProfile':''});
     }
    ...
    const tileView = async () => {
        ...
        if($("body").hasClass('TileView') === true){
            ...
            if(userColLength === 4){
                $('.cl_user_list').append(
                    `<div class="cl_user_list_box">
                        <div class="cl_user_list_box_videos">
                            <span class="circle_remote_participant d-none"></span>
                            <span class="circle_remote_participant d-none"></span> 
                        </div>
                    </div>`
                    );
    ...
    const getCaptions = (latestCaptions) => {
        ...
        if(isSubtitle && (isAudioMuted == false || oldsubtitleSegment == latestCaptions[latestCaptions.length - 1].subtitleSegment) ) {
            //console.log('captions[captions.length - 1].user_id : ',captions[captions.length - 1].user_id);
            //console.log('userId : ',userId);
            oldsubtitleSegment = latestCaptions[latestCaptions.length - 1].subtitleSegment
            if(captions.length > 0 && captions[captions.length - 1].user_id === userId ) {
                captions.splice(captions.length - 1,1);
                captions.push({'user_id':userId,'username':latestCaptions[latestCaptions.length - 1].username,'captions':latestCaptions[latestCaptions.length - 1].captions, 'currentSubtitle':latestCaptions[latestCaptions.length - 1].currentSubtitle, 'subtitleFlag':latestCaptions[latestCaptions.length - 1].subtitleFlag, 'subtitleSegment':latestCaptions[latestCaptions.length - 1].subtitleSegment, 'userProfile':latestCaptions[latestCaptions.length - 1].userProfile});
                removeOldSTT = false;
                localStorage.setItem('removeOldSTT',0);
            } else {
                removeOldSTT = true;
            ...
            socket.emit('subtitle', {'sttSentence':latestCaptions[latestCaptions.length - 1].captions, 'userName':latestCaptions[latestCaptions.length - 1].username, 'isSubtitle':isSubtitle, 'remoteUserId':userId, 'subtitle':latestCaptions[latestCaptions.length - 1].currentSubtitle, 'subtitleFlag':latestCaptions[latestCaptions.length - 1].subtitleFlag, 'subtitleSegment':latestCaptions[latestCaptions.length - 1].subtitleSegment,'remoteUserProfile':latestCaptions[latestCaptions.length - 1].userProfile});
    ...
    return(
        ...
        {
            (
                <STTComponent getCaptions={getCaptions} isSubtitle={isSubtitle} isAudioMuted={isAudioMuted} removeOldSTT={removeOldSTT}/>
            )
        }
        {
            prelaunchScreen === false ? (
                <>
                    {
                        recordingHandler && <Recorder handleRec={setRecordingHandler} stateChanger={setLeaveDisable} action={recordingHandler} meetingId={currentMeetingId} stateLoader={setSpinner} />
                    }
                    <div style={{height:'100vh',overflow:'hidden'}} className="cl_meeting_videos_main">
                        {/* Top  Meeting Menu  */}
                            ...
                            <div className="cl_main_meeting_box">
                                ...
                                <div className="cl_main_home_box">
                                    ...
                                    <div className="cl_user_list_box_control">
                                        ...
                                        <h4 style={{color:userRole=='Teacher'?'#1C98E5':'#FFF'}}>{userName}{userRole=='Teacher'?' (선생님)': userRole=='Student'?' (학생)':''}</h4>
                                    </div>
                                    {/* {
                                        isSubtitle && (
                                    ...
                                    {
                                        isSubtitle && (
                                        <div className="cl_main_noti_box">
                                        {
                                            captions && captions.map((singleCaption,i) => {
                                                if(singleCaption.subtitleFlag == false){
                                                    return(<div key={i} className="singlecaption" style={{display: 'flex', margin: '10px', paddingRight: '70px', alignItems: 'self-start'}}><img style={{borderRadius: '50%', width: '40px', height: '40px', maxWidth: 'unset', position: 'relative', right: '-6px', margin: '15px 0 0 0'}} src={singleCaption.userProfile} ></img><h6 style={{ margin: '10px' , paddingLeft: '10px'}}>{singleCaption.username} <br/> {singleCaption.captions + singleCaption.currentSubtitle}</h6></div>);
                                                }else{
                                                    return(<div key={i} className="singlecaption" style={{display: 'flex', margin: '10px', alignItems: 'self-start'}}><img style={{borderRadius: '50%', width: '40px', height: '40px', maxWidth: 'unset', position: 'relative', right: '-6px', margin: '15px 0 0 0'}} src={singleCaption.userProfile} ></img><h6 style={{ margin: '10px', paddingLeft: '10px' }}>{singleCaption.username} <br/> {singleCaption.captions}</h6></div>);
                                                }
                                            })
                                        }
                                        </div>
                            ...
                            <div className="meeting_menu meeting_menu_bottom_left">
                                <ul>
                                    ...
                                    </li>
                                    {
                                        localStorage.getItem('user_role') === "Teacher" && (
                                            <li>
                                                <a href="#." onClick={() => subtitleHandler()}>
                                ...
                                <ul>
                                            ...
                                            <a href="#." onClick={clickrecoding}>
                                                {
                                                  ...
                                                  <>
                                                <svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path id="stoprecording_svg" className="active_svg" d="M22 0C26.3512 0 30.6047 1.29028 34.2225 3.70767C37.8404 6.12506 40.6602 9.56099 42.3253 13.581C43.9905 17.6009 44.4261 22.0244 43.5773 26.292C42.7284 30.5596 40.6331 34.4796 37.5564 37.5564C34.4796 40.6331 30.5596 42.7284 26.292 43.5773C22.0244 44.4261 17.6009 43.9905 13.581 42.3253C9.56099 40.6602 6.12506 37.8404 3.70767 34.2225C1.29028 30.6047 0 26.3512 0 22C0 16.1652 2.31785 10.5695 6.44365 6.44365C10.5695 2.31785 16.1652 0 22 0V0Z" fill="#464958" />
                                                ...
                                                <><svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path id="startrecording_svg" className="active_svg" d="M22 0C26.3512 0 30.6047 1.29028 34.2225 3.70767C37.8404 6.12506 40.6602 9.56099 42.3253 13.581C43.9905 17.6009 44.4261 22.0244 43.5773 26.292C42.7284 30.5596 40.6331 34.4796 37.5564 37.5564C34.4796 40.6331 30.5596 42.7284 26.292 43.5773C22.0244 44.4261 17.6009 43.9905 13.581 42.3253C9.56099 40.6602 6.12506 37.8404 3.70767 34.2225C1.29028 30.6047 0 26.3512 0 22C0 16.1652 2.31785 10.5695 6.44365 6.44365C10.5695 2.31785 16.1652 0 22 0V0Z" fill="#464958" />
                                                    <path className="normal_svg" d="M22 26.3281C22.7911 26.3281 23.5645 26.0935 24.2223 25.654C24.8801 25.2145 25.3928 24.5898 25.6955 23.8589C25.9983 23.128 26.0775 22.3237 25.9231 21.5478C25.7688 20.7718 25.3878 20.0591 24.8284 19.4997C24.269 18.9403 23.5563 18.5593 22.7804 18.405C22.0044 18.2506 21.2002 18.3299 20.4693 18.6326C19.7384 18.9354 19.1136 19.4481 18.6741 20.1058C18.2346 20.7636 18 21.537 18 22.3281C18 23.389 18.4214 24.4064 19.1716 25.1566C19.9217 25.9067 20.9391 26.3281 22 26.3281Z" fill="white" />
                                                    <path className="normal_svg" fillRule="evenodd" clipRule="evenodd" d="M32 22.3281C32 24.3059 31.4135 26.2393 30.3147 27.8838C29.2159 29.5283 27.6541 30.81 25.8268 31.5669C23.9996 32.3238 21.9889 32.5218 20.0491 32.136C18.1093 31.7501 16.3275 30.7977 14.9289 29.3992C13.5304 28.0007 12.578 26.2188 12.1922 24.279C11.8063 22.3392 12.0043 20.3286 12.7612 18.5013C13.5181 16.674 14.7998 15.1122 16.4443 14.0134C18.0888 12.9146 20.0222 12.3281 22 12.3281C23.3132 12.3281 24.6136 12.5868 25.8268 13.0893C27.0401 13.5919 28.1425 14.3285 29.0711 15.2571C29.9997 16.1856 30.7363 17.288 31.2388 18.5013C31.7413 19.7145 32 21.0149 32 22.3281V22.3281ZM30 22.3281C30 23.9104 29.5308 25.4571 28.6518 26.7727C27.7727 28.0883 26.5233 29.1137 25.0615 29.7192C23.5997 30.3247 21.9911 30.4831 20.4393 30.1744C18.8874 29.8657 17.462 29.1038 16.3432 27.985C15.2243 26.8662 14.4624 25.4407 14.1537 23.8888C13.845 22.337 14.0035 20.7285 14.609 19.2667C15.2145 17.8048 16.2398 16.5554 17.5554 15.6764C18.871 14.7973 20.4178 14.3281 22 14.3281C24.1217 14.3281 26.1566 15.171 27.6569 16.6713C29.1572 18.1716 30 20.2064 30 22.3281Z" fill="white" />
                                                    <path className="normal_svg" d="M32 31.8281L25.25 23.9531L14 10.8281" stroke="white" strokeWidth="2" />
                                                    <path className="normal_svg" d="M30.908 33.1711L23.817 24.9801L12 11.3281" stroke="#424649" strokeWidth="2" />
                                    ...
                                    }
                                    {
                                        localStorage.getItem('user_role') === "Teacher" && (
                                            ...
                                            <li>
                                                <button className="cl_button" id="togglePipButton"
                                                style={{ 'background-color': 'transparent',color: 'transparent', border: 'transparent', padding: '0px' }}
                                                >PIP</button>
                                            </li>
                                        )
                                    }
                                    
                                    {
                                        localStorage.getItem('user_role') === "Teacher" && (
                                            <li>
                                                 <div id='outer-pip'>
                                                 <video className='remote-video-styles video-flip' autoPlay id='pip-video' />
                                                 </div>
                                            </li>
                                        )
                                    }
                                ...
                                <ul>
                                    ...
                                    {spinner ?
	                                    <Loader  loading={spinner} /> :
                                         <li className= {leaveDisable ? 'disabled':''}>
                                            <a href="#." onClick={unload}>
                                                ...
                                                <div className="icon_name">
                                                        {leaveDisable ? 'Recording in-progress!':'Leave Call'}
                                                </div>
                                            </a>
                                        </li>
                                    }
