...
function MeetingUI(props) {
    ...
    /* global $, JitsiMeetJS */    
    const options = {
        serviceUrl: 'https://unoxpro.unoxportal.com/http-bind',
        hosts: {
            domain: 'unoxpro.unoxportal.com',
            muc: 'conference.unoxpro.unoxportal.com'
        },
        resolution: 1080,
                        ...
                        $('.cl_user_list').append(
                            `<div class="cl_user_list_box" id='remote-user-${participant}'>
                                <div class="cl_user_list_box_videos">
                                    ...
                                    <video autoplay='1' class="${participant}video" id='${participant}video${idx}' onclick="$('body').toggleClass('fullscreen'); if($('body').hasClass('fullscreen') === true){$('.cl_user_list').css('width','100%');}else{$('.cl_user_list').css('width','50%');}"/>
                                </div>
                                <div class="cl_user_list_box_control">
                                    <div class="mute_icon">
                                        <svg class="audio-img-${participant} width="10" height="11" viewBox="0 0 10 11" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    */    
    ...
    const switchVideo = () => {
        ...
        .then(tracks => {
            ...
            // Screeen + Video of teacher to student
            if (tracks[0].videoType === 'desktop') {
                //socket.emit('share-screen',{data:true});
            }else{
                if(isVideoMuted === true){
                    tracks[0].mute();
                }
                                ...
                                <div className="user_chat_box_btm_main">
                                    {
                                        localStorage.getItem('user_role') === "Teacher" && (
                                            <div className="user_chat_box_btm">
                                                ...
                                                <input type="file" id="meeting_document" style={{visibility:"hidden"}} onChange={uploadHandler}/>
                                                {
