    ...
    useEffect(() => {
        ...
        if (socket) {
            ...
            socket.on('subtitle-data', ({ subtitle, userName }) => {
                if(userRole == "Student"){
                    if (subtitle && isAudioMuted == false) {
            ...
            socket.on('on-document-counter',({uploadeddocumentcount})=> {
                console.log("uploadeddocumentcount",uploadeddocumentcount);
            });
            ...
            socket.on('on-remote-participant-handler',({requested_participant_id})=> {
                ...
                console.log("REMOTE USER AUDIO:",requested_participant_id);
                console.log(room.p2pDominantSpeakerDetection.myUserID);
    ...
    const onConnectionSuccess= () => {
        try {
            ...
            room.on(JitsiMeetJS.events.conference.TRACK_MUTE_CHANGED, track => {
                ...
                // This code used for manage user mute / unmute when adding remove participents
                /*if(track.isMuted()){
                }*/
            });
            ...
            let userRole = localStorage.getItem('user_role');
            let userName = localStorage.getItem('user_name');
            room.setDisplayName(userRole + '#' + userName + '#' + initialName);
            room.join();
                    ...
                    $('.cl_user_list').append(
                        `<div class="cl_user_list_box" id='remote-user-${participant}'>
                            <div class="cl_user_list_box_videos">
                                <span class="circle_remote_participant d-none" id="${participant}img">${participantName[2]}<span></span></span>
                ...
                if (localTracks[i].type === "audio") {
                    try {
                        if (isAudioMuted === true) {
                            ...
                            $("#local_audio").css("fill", "#464958");
                            isAudioMuted = false;
                        } else {
                            localTracks[i].mute();
                            $(".cl_main_noti_box").addClass('d-none');
                            $("#local_audio").css("fill", "#ff0000");
                            isAudioMuted = true;
                        }
                    } catch (err) {
                        console.log(err);
    ...
    */    
    ...
    const manageChat = async () => {
        ...
        if (chatMessage) {
            ...
            $("#remote_chat").append('<span className="time_chat" id="my_name">You <strong>'+chatTime+'</strong></span><p>'+chatMessage+'</p>');
            
            // Scroll to bottom
            $("#user_chat_panel").animate({ scrollTop: $('#user_chat_panel').prop("scrollHeight") }, 1000);

            // Send message to conference
            socket.emit("send-message", { message: chatMessage, messagetime: chatTime, name: localStorage.getItem('user_name')});
                    ...
                    <div className="cl_meeting_videos_main">
                        ...
                        <div className="meeing_video_body">
                                    ...
                                    <ul className="participant-list">
                                        <div className="='parent'">
                                        {
                                            remoteParticipantList && remoteParticipantList.map((track) => (
                                                <div className="users" key={track._id}>{track._displayName.split('#')[1]} <button onClick={() => presentToMeeting(track._id,'show')}>Present</button></div>
                                            ))
                                        }
                                        </div>
                                    </ul>
                            ...
                            <div className="user_chat_box">
                                ...
                                <div className="user_chat_box_btm_main">
                                   <div className="user_chat_box_btm">
                                      <input type="text" id="send_mesage" placeholder="Type message" autoComplete="off" onChange={(e) => setChatMessage(e.target.value)}/> 
                                      <button className="send_btn" onClick={manageChat}>
                                         <svg xmlns="http://www.w3.org/2000/svg" width="30.548" height="30.548" viewBox="0 0 47.548 46.669">
                            ...
                            <div className="document-list-wrapper">
                                        ...
                                        <div className="='parent'">
                                            {
                                                documentList && documentList.map((doc,i) => (
                                                    <div className="users" key={i}>{doc.documentTitle}
                                                        <div className="actions">
                                                            <a href={doc.url} target="_blank">Download</a>
                                                            <a href="#." onClick={() => deleteDocument(doc.documentId)}>Delete</a>
                                                        </div>
                                                    </div>
                                                ))
                                            }
                                ...
                                <div className="user_chat_box_btm_main">
                                   <div className="user_chat_box_btm">
                                      <input type="file" id="meeting_document" onChange={uploadHandler}/>
                                        {
                            ...
                            <div className="cl_main_meeting_box">
                                <div className="cl_main_home_box">
                                    <div className="cl_main_home_box_video">
                                        <span className="local_video_mute_img Name_value d-none">{initialName}<span></span></span>
                                    </div>
                                    <div className="cl_user_list_box_control">
                                        <div className="mute_icon">
                                    ...
                                    <li>
                                        <a href="#." onClick={muteHandler}>
                                            <svg width="44" height="44" viewBox="0 0 44 44" xmlns="http://www.w3.org/2000/svg">
                                                <path id="local_audio" d="M22 44C34.1503 44 44 34.1503 44 22C44 9.84974 34.1503 0 22 0C9.84974 0 0 9.84974 0 22C0 34.1503 9.84974 44 22 44Z" className="active_svg" fill="#287BD7"/>
                                                <path className="normal_svg" d="M31.9567 29.8303L26.9886 24.8463C27.4066 23.8055 27.6202 22.6939 27.6176 21.5723V19.6113C27.6349 19.4588 27.5925 19.3056 27.4993 19.1837C27.4061 19.0617 27.2693 18.9806 27.1176 18.9573H26.6176C26.4659 18.9804 26.3289 19.0614 26.2357 19.1834C26.1424 19.3054 26.1001 19.4588 26.1176 19.6113V21.5723C26.1175 22.2698 26.0033 22.9627 25.7796 23.6234L24.9437 22.7853C25.0499 22.3896 25.1056 21.982 25.1097 21.5723V15.0343C25.1097 12.8683 23.7566 11.1113 22.0866 11.1113C20.4166 11.1113 19.0637 12.8683 19.0637 15.0343V16.8883L13.4417 11.2493C13.3931 11.1978 13.3327 11.159 13.2657 11.1364C13.1987 11.1137 13.1271 11.1078 13.0573 11.1193C12.9875 11.1307 12.9215 11.1592 12.8653 11.2021C12.809 11.245 12.7642 11.3011 12.7347 11.3654L12.1166 12.3973C12.0337 12.5402 11.9976 12.7055 12.0134 12.8699C12.0292 13.0344 12.0961 13.1898 12.2047 13.3143L30.7297 31.8963C30.7781 31.9478 30.8385 31.9867 30.9054 32.0095C30.9724 32.0322 31.0439 32.0381 31.1137 32.0268C31.1835 32.0154 31.2495 31.9871 31.3058 31.9443C31.3621 31.9015 31.407 31.8455 31.4366 31.7813L32.0557 30.7493C32.1376 30.6051 32.1721 30.4388 32.1544 30.2739C32.1366 30.1089 32.0674 29.9538 31.9567 29.8303V29.8303ZM24.5947 30.0714H22.8317V28.6913C23.1977 28.6223 23.5537 28.5081 23.8916 28.3513L22.3127 26.7693C22.0998 26.7938 21.885 26.7965 21.6716 26.7773C19.9136 26.5553 18.5626 24.7893 18.1716 22.6163L16.5336 20.9724V21.2523C16.5336 24.9143 18.5477 28.1803 21.3197 28.6763V30.0764H19.5567C19.4049 30.0994 19.2679 30.1805 19.1747 30.3024C19.0814 30.4244 19.0391 30.5778 19.0567 30.7303V31.3843C19.0394 31.5368 19.0818 31.6901 19.175 31.812C19.2682 31.9339 19.405 32.015 19.5567 32.0383H24.5947C24.7463 32.015 24.8831 31.9339 24.9763 31.812C25.0695 31.6901 25.1119 31.5368 25.0947 31.3843V30.7303C25.1136 30.577 25.0719 30.4223 24.9785 30.2993C24.8851 30.1762 24.7474 30.0944 24.5947 30.0714V30.0714Z" fill="#fff"/>
                                            </svg>
